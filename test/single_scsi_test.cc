...
static std::pair<int, std::string> file_type_list[] = {
    {FT_OTHER, "FT_OTHER"}, {FT_SG, "FT_SG"},       {FT_RAW, "FT_RAW"},
    {FT_BLOCK, "FT_BLOCK"}, {FT_ERROR, "FT_ERROR"},
};
...
static int sg_build_scsi_cdb(uint8_t *cdbp, int cdb_sz, unsigned int blocks,
                             int64_t start_block, bool is_verify,
 ...
 *
 **/
static int sg_write(int sg_fd, uint8_t *buff, int blocks, int64_t to_block,
  ...
  assert(0 == sg_build_scsi_cdb(wrCmd, cdbsz, blocks, to_block, true /*verify*/,
                                true /*write_true*/, fua, dpo));
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < total_write_test; i++) {
    ...
    bool direct_io = true;
    auto ret = SCSI::sg_write(
        fd, buf, (op_size + overlap_size) / kBlockSize /*block per write*/,
        (write_pos - overlap_size) / kBlockSize /*write_to_block_pos*/,
        kBlockSize /*block_size*/, 10 /*SCSI cmd length*/, false /*FUA*/,
        false /*dpo*/, &direct_io);
